...
Иногда переводимый текст не относится ни к секции, ни к полю ресурса, ни к ТВ — это просто надпись в вёрстке (подпись в шапке, текст кнопки, дисклеймер в подвале), которую нужно переводить. Помечается атрибутом **`data-mpc-lexicon="топик:ключ"`** на любом **не-секционном** элементе: содержимое тега становится лексиконным ключом со своим именем и своим файлом-топиком.
...
Содержимое (`Скидки до конца месяца`) уезжает в файл лексикона топика `blocks` под ключом `promo_title`, а на месте встаёт плейсхолдер `{'promo_title' | lexicon}`, который на рендере подставляет значение текущего языка.

Топик можно не указывать — тогда берётся первый из настройки `mpc_arbitrary_lexicon_topics`:

```html
<!-- вход -->
<span data-mpc-lexicon="promo_title">Скидки до конца месяца</span>
```

::: danger Фича работает только при включённых лексиконах
Произвольный ключ — часть механизма лексиконов: он действует, только если `mpc_use_lexicons = Да`. Кроме того, использованный топик обязан быть перечислен в настройке `mpc_arbitrary_lexicon_topics`, иначе на сайте `{'ключ' | lexicon}` вернёт пусто.
:::

::: warning На секции `data-mpc-lexicon` означает другое
На элементе с `data-mpc-section` этот же атрибут — [префикс лексикона секции](#дополнительные-атрибуты-секции), не произвольный ключ. Произвольный ключ — только на элементах **без** `data-mpc-section`.
:::

Полный разбор (что именно вырезается и как санируется, выбор топика, правка из визуального редактора, готча с `<div>` внутри `<p>`) — в разделе [Лексиконы и мультиязычность](lexicons#произвольный-лексиконный-ключ).
