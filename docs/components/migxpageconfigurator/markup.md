...
- **`data-mpc-if`** — условный вывод **всей секции** (см. [Общие атрибуты](#общие-атрибуты));
...
Эти атрибуты не привязаны к одному типу элемента — их можно навесить на разные размечаемые сущности: поля (`data-mpc-field`), секции (`data-mpc-section`), вызовы сниппетов (`data-mpc-snippet`), поля ресурса (`data-mpc-rfield`), TV (`data-mpc-tv`) и настройки (`data-mpc-info`).

**`data-mpc-if`** — выводить элемент только при выполнении условия. В значении — условие на Fenom; если оставить значение пустым, условием станет само значение поля (выводится, когда непустое):

```html
<a data-mpc-field="link" data-mpc-if="$link" href="/contacts">Связаться</a>
```

Здесь кнопка попадёт в результат, только если поле `link` заполнено. Условие можно задать и явно — например `data-mpc-if="$price > 0"`. Точно так же атрибут оборачивает в условие **секцию** (выводить секцию целиком только при условии) и **вызов сниппета**.
