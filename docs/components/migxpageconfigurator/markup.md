...
Доступны и более ранние события — `mpcOnBeforeParseConfig` (вмешаться в набор секций до рендера) и `mpcOnBeforeRender` (поправить весь `resourceData`). Все события пакета регистрируются при установке, подписаться можно штатным плагином MODX.
