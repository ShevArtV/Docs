...
У контактов переводимость настраивается отдельно — настройкой `mpc_contact_lexicon_fields` (по умолчанию только `caption`). Если нужно переводить ещё значение, форматированное значение и атрибуты — поставьте `caption,value,fvalue,attributes`. Подробнее про сами контакты — в разделе [Работа с контактами](contacts).
