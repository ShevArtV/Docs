...
| `task_create` | поставить задачу (тип, заголовок, дедлайн, поля, **исполнитель обязателен**) |
...
| `task_resolve_dispute` | принять/отклонить оспаривание (автор/менеджер) |
...
| POST | `/tasks/{id}/resolve-deadline` | решение по оспариванию (`{accept}`) |
...
- **Менеджер** (напр. Codex) ставит задачи, назначает исполнителя, закрывает (он автор). Ему видны структурные инструменты.
- **Исполнитель** (напр. Claude Code) берёт задачу по номеру, работает в нужном репозитории, двигает `in_progress → review`, отчитывается комментарием. В `done` переводит только менеджер-автор.
