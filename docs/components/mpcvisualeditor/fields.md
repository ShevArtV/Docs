...
Иногда поле есть в конфигурации, но не видно на странице — оно вырезано из вёрстки (`data-mpc-remove`) или вспомогательное. Такие поля нельзя кликнуть на странице (их там нет), но править их всё равно можно — через **панель скрытых полей**.

Открыть панель можно двумя способами:

- **кнопкой «✎ поля»** у секции в [панели «Секции»](sections) (в сайдбаре) — покажет все скрытые и вспомогательные поля этой секции;
- **кнопкой-триггером** на самом блоке — покажет скрытые поля именно этого блока.

В панели поля правятся теми же редакторами, что и обычные (текст, картинка, цвет и т.д.). Здесь же доступны **поля-списки** (MIGX): можно править строки, поля внутри строк и вложенные списки любой глубины — даже если самого списка нет в видимой вёрстке. Картинки и медиа внутри строк открываются своими редакторами, как и на странице.
