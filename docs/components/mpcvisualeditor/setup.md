...
| `mpcve_toolbar_position` | `top` | где закреплена панель редактора: `top` — вверху страницы, `bottom` — внизу (см. ниже) |
...
Панель (тулбар) редактора по умолчанию закреплена **вверху** страницы. Настройкой `mpcve_toolbar_position` её можно перенести **вниз** (`bottom`) — тогда вниз уезжает и сама панель, и баннер блокировки страницы, а отступ страницы подстраивается, чтобы панель ничего не перекрывала.

Панель можно **свернуть** — кнопкой на самой панели она сворачивается в компактный значок в левом углу, освобождая место. Развернуть — клик по этому значку. Свёрнутое/развёрнутое состояние запоминается в cookie (`mpcve_collapsed`) и переживает перезагрузку страницы.
